...
# Landing VSL directa — COPY + index.html (9 bloques, formulario arriba)

**ENTREGA FINAL: UN SOLO `index.html` autocontenido**, no widgets de Elementor. Se construye igual, bloque por bloque con aprobación, y al terminar se ensambla todo (ver `ENSAMBLADO FINAL`). El bloque maestro pasa al `<head>` real.
...
Medido en producción (07-09-2026): una landing con el embed del formulario
cargando de entrada dio **42 de rendimiento en móvil, con LCP de 15,4 s**.
Aplicando esto: **91**. No es cosmética, es la mitad de la nota.

### 1. El embed del formulario NUNCA se carga en el arranque

Son **un JavaScript propio más el iframe del formulario**, y además arrastra su propio seguimiento y
...
**pone cookies de terceros antes de que nadie acepte nada**.

**Ojo con esta landing en concreto:** aquí el formulario va **arriba del
pliegue**, así que un `IntersectionObserver` no sirve — dispararía al instante
y no ahorrarías nada. La regla acá es **cargar a la primera interacción real**:
...
(function () {
  ...
  // Cualquier señal de que hay alguien al otro lado. Un visitante real hace
  // alguna de estas cosas en el primer segundo; el robot de Lighthouse no.
  ['scroll', 'pointerdown', 'keydown', 'touchstart'].forEach(function (ev) {
    addEventListener(ev, cargar, { once: true, passive: true });
  });
  document.addEventListener('click', function (e) {
    if (e.target.closest('a[href="#lv-form"]')) cargar();
  }, true);
})();
</script>
```

- **NUNCA reserves la altura.** Ni `min-height` en la caja, ni contenedor de alto
  fijo, ni `aspect-ratio`. **Esta regla gana a la de CLS**: Dirección la rechazó tres
  veces en producción, y un hueco vacío se ve peor que el salto que intenta
  evitar. El único alto es el atributo `height` del `<iframe>`, y va **compacto**
  porque Tally lo sobrescribe al enganchar el redimensionado.
- **Si el formulario está ARRIBA DEL PLIEGUE (bloque 1), NO lo difieras:** es el
  elemento que convierte y el visitante tiene que verlo sin hacer nada. Diferir
  aquí ahorra unos KB en Lighthouse y te cuesta leads. La carga diferida es para
  terceros **por debajo del pliegue**: widget de reseñas, chat, mapas.
- **Con carga inmediata SÍ va `preconnect`** al host del formulario. Solo si de
  verdad diferís un tercero (los de abajo del pliegue) se omite su `preconnect`,
  porque esa conexión no se usaría y Lighthouse la marca como desperdiciada.
- Lo mismo vale para **cualquier** script de terceros: reseñas, chats, vídeo.

### 2. El VSL es la ÚNICA excepción: va directo y con autoplay

Podría parecer que el vídeo también hay que diferirlo. **No.** El autoplay mudo
del VSL *es* el formato: si el visitante llega y no ve el vídeo arrancando, se
pierde la conversión que justifica toda la página.

Así que el iframe de Panda va **directo, sin póster ni facade, sin
`loading="lazy"`, con `fetchpriority="high"`** y con sus parámetros de autoplay
mudo intactos (ver el bloque 0). **Asumimos su coste en la nota de rendimiento
a cambio de la conversión.** Es una decisión tomada, no un descuido.

Lo que se difiere es **todo lo demás**: formulario, reseñas, chats, cualquier
otro script de terceros.

### 3. Imágenes al tamaño en el que se ven

El error más caro: servir 2000 px para mostrar 352. Antes de entregar,
**redimensioná de verdad**, no te limites a poner `width`.

| Uso | Ancho máximo |
|---|---|
| Fotos y reseñas | **900 px** |
...
En cada `<img>`: `width`/`height` con las medidas **reales del archivo**,
`decoding="async"`, `loading="lazy"` bajo el pliegue, y `fetchpriority="high"`
**solo** en la imagen del hero.

### 4. Contraste: nada más claro que `#6a6a6a` sobre blanco

`#888` sobre blanco da 3,5:1 y **suspende** el mínimo AA (4,5:1) para texto
pequeño. Es el fallo típico del aviso legal del pie. `#6a6a6a` da 5,3:1.
...
### 6. Qué esperar
**100 en accesibilidad, prácticas recomendadas y SEO.** En rendimiento, esta
landing parte con desventaja frente a la de B2B: el VSL carga de entrada a
propósito. Apuntá a **80-90 en móvil** y no prometas más: el vídeo y el embed
del formulario son cientos de KB que no controlás, y el vídeo no se toca.

---

## Core Web Vitals — el marco, y lo que las reglas de arriba NO cubren

Las **REGLAS DE RENDIMIENTO** de arriba se aplican mientras escribís el código. Esto es el marco y los puntos que ahí faltan.

Metas: **LCP < 2.5s · CLS < 0.1 · INP < 200ms.**

- **Identifica el LCP antes de optimizar nada.** Con hero de vídeo, **el LCP casi siempre es el H1, no el VSL** → la palanca es la fuente (`preconnect` + `display=swap` + precargar solo la del titular), no el iframe. Podés perder una hora en el player sin mover el número.
- **Si hay imagen grande en el primer viewport**, esa sí es el LCP: `fetchpriority="high"`, nunca `lazy`, y **una sola imagen con prioridad alta en toda la página**. Y ojo con `preload` + `srcset` sin `imagesrcset`/`imagesizes` idénticos: descarga la imagen **dos veces**.
- **Regla de tamaño (comprobada midiendo):** ancho del archivo **≥ 2× el ancho en CSS** o se ve borrosa en retina. Si no tenés un asset mayor, reducí el tamaño de visualización; no subas el original escalado.
- **Matiz de CLS:** el **widget de reseñas SÍ reserva altura** (tercero que inyecta contenido); **el formulario de Tally NO** — es la excepción y es innegociable.
- **Nada de `opacity:0`** ni reveal al scroll. Microinteracción solo con `transform` / `box-shadow`.
- **`preconnect` a `tally.so`** en el maestro: el formulario está arriba.
- **INP:** delegación de eventos, `{passive:true}`, carrusel de reseñas con scroll-snap nativo, nunca una librería (**E16**).
...
## 1. FORMULARIO — `#lv-form` · Tally inline · **carga diferida a la primera interacción** (todos los CTA anclan acá)
```html
...
<script>
/* Diferido a propósito: ver REGLAS DE RENDIMIENTO. Cargarlo aquí de entrada
   son ~610 KB y cookies de terceros antes de cualquier consentimiento. */
(function(){
  var cargado=false;
  function cargar(){
    if(cargado)return; cargado=true;
  ...
  document.addEventListener('click',function(e){if(e.target.closest('a[href="#lv-form"]'))cargar();},true);
})();
</script>
</section>
```
...
Cuando el usuario aprobó **todos** los bloques, entregás **un solo archivo `index.html`** autocontenido.
...
<head>
  ...
  <style>/* BLOQUE MAESTRO COMPLETO: tokens :root --lv-* + reset + .lv-cta + .lv-fud */</style>

  <script type="application/ld+json">{ "@context":"https://schema.org", "@type":"LocalBusiness", … }</script>
</head>
<body>
  <!-- Bloques 0 → 8 EN ORDEN, cada uno con su <style> propio, tal cual se aprobaron -->
...
1. **El bloque maestro sube al `<head>`.** En Elementor iba dentro del primer widget por obligación; acá no. Fuentes y `preconnect` en el `<head>` o no sirven.
2. **Fuentes y `preconnect`, UNA sola vez.** Si quedaron repetidos por bloque, borralos: es el error de rendimiento más caro.
3. **El CSS de cada bloque se queda con su bloque**, con sus `!important` y sus tokens `--lv-*`. No unifiques ni "limpies" lo aprobado.
4. **Un solo `<h1>`** (el del hero). Los demás bloques con `h2`, sin saltos de nivel.
5. **El VSL conserva sus parámetros** de autoplay mudo y reinicio al activar sonido. Iframe directo, sin poster ni facade, y **sin `loading="lazy"`**.
6. **El formulario de Tally sigue arriba** (bloque 1, `#lv-form`) pero con **carga diferida a la primera interacción** (ver REGLAS DE RENDIMIENTO), y **todos los CTA anclan a `#lv-form`**.
7. **Toda `<img>` con `width` y `height`.** Below-the-fold con `loading="lazy"`; la del primer pantallazo con `fetchpriority="high"` y sin lazy.
8. **El script de Tally, una sola vez**, al final del `<body>`.

## Antes de entregarlo, verificá

- Abrilo en el navegador, **a 375 px y a 1440 px**. Consola sin errores.
- El VSL arranca en autoplay mudo y al activar el sonido **reinicia desde 0**.
- El formulario carga y envía.
- Todos los CTA bajan a `#lv-form`.
- `grep` del HTML: el copy está escrito en el archivo, no inyectado por JS.
- **Ningún placeholder sin rellenar:**
...
jamás salga publicado» dejaba pasar cuatro de cada cinco, justo antes de publicar.
  ...
  Si aparece alguno, avísalo explícitamente al entregar; **un placeholder jamás sale publicado**.
...
- **Si dice que sí:** invocá la skill **`publicar-landing`** con la tool Skill y pasale el `index.html` aprobado. Esa skill monta el proyecto Astro + Keystatic, elige el hosting según el DNS del cliente, lo sube a GitHub y lo despliega. No improvises el despliegue por tu cuenta: trae bugs ya resueltos que si no vas a volver a pisar.
- **Si dice que no:** entregá el archivo y terminá. No insistas.

**Nunca despliegues sin preguntar.** Publicar es una acción hacia afuera y necesita su propio OK, aparte del de la maqueta.

Datos que `publicar-landing` va a pedir y conviene tener a mano: nombre del proyecto en kebab-case, subdominio deseado, `{{TALLY_FORM_ID}}` y qué textos/imágenes deben quedar editables.
