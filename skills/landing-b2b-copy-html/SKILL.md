...
>
> **Restricción dura:** pueden cambiar el ACABADO, nunca la ARQUITECTURA funcional (full-bleed, tokens `--lc-*`, `.lc-frame`, formulario Tally, `!important`, rendimiento, orden de bloques).
>
...
1. **PASO 0 — Brief.** Pedí el brief completo (ver PARTE A · Paso 0) + la config de maquetación (ver PARTE B: plataforma, VSL sí/no, `{{TALLY_FORM_ID}}`, colores/fuentes de marca). No avances sin lo esencial (servicio/audiencia, promesa central, urgencia real sí/no).
...
**El test:** si el lector solo mira titulares y negritas, ¿entiende la oferta completa? Si no, el titular no dice nada.

## 🔴 CÓMO SE ESCRIBE UN DOLOR
**Cada uno tiene que ser una ESCENA que el lector haya vivido**, no una categoría abstracta.
...
2. **Con su costo nombrado** (horas, dinero, riesgo, tranquilidad), **al final del bullet**.
3. **En sus palabras (VoC).** No traduzcas "me tienen en la oscuridad" a "falta de visibilidad".
...
El error habitual es listar **lo que vos hacés**. Al lector le importa **cómo queda él**.
...
1. **Principios de Ogilvy** — titulares, subtítulos, claridad y persuasión por beneficio; ya están destilados **más abajo en este mismo archivo** (esta skill es autocontenida). El fichero `../fundamentos-copy/references/ogilvy-principios.md` que se citaba aquí **no existe en esta skill**: era un puntero roto. Si hace falta la fuente real: `../fundamentos-copy/references/ogilvy-reglas-reales.md` (16 checks O1-O16) y el PDF del libro. (El PDF completo `../fundamentos-copy/references/ogilvy-on-advertising.pdf` (una sola copia para todo el agente) está disponible para citas profundas; leelo con `pages` solo si necesitás una cita literal.)
...
Pedí: qué servicio y a quién (perfil de cliente ideal); promesa central + garantía de expertise/seguridad; si hay descuento/urgencia REAL con fecha límite (si no, se omite el countdown); los 5 problemas de "hacerlo solo" + sus 5 contrapartes resueltas; el costo emocional/de riesgo de no resolverlo; los 4-5 pasos reales del proceso; 4-5 situaciones específicas del cliente ideal; prueba social real (testimonios, cifras, credenciales — si no hay, avisá y dejá placeholders marcados como ejemplo, nunca inventados); el expertise real del equipo; y las objeciones reales para el FAQ. No bloquees todo por un dato menor: asumí un valor razonable, decilo y seguí. Pausá solo si falta servicio/audiencia, promesa central, o no está claro si hay urgencia real.
...
**Genera:**
- Subtítulo columna dolor ("Si lo haces solo…") + subtítulo columna alivio ("Con el apoyo correcto…").
- 5 bullets EN ESPEJO: cada dolor de la izquierda tiene su contraparte exacta resuelta a la derecha, mismo orden.
- Párrafo de cierre (2-3 líneas) que resume la propuesta de valor y menciona de nuevo el segmento.
- Texto del CTA + microlínea de quita-miedos.

**Nunca hagas esto:** no dejes un bullet de dolor sin su contraparte exacta (deben leerse como espejo punto a punto).
...
- Parte A — "Con [marca], puedes:" + 5 beneficios concretos (verbos: confirmar, elegir, preparar, gestionar, avanzar).
